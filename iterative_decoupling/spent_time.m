...
methods = {'kdl','lma','decoup','ikfast'};
labels = {'N--R','L--M','I--D','IKFast'};
...
for i = 1:cols
  ...
  anova_data = [anova_data;results(i).time * 1000];
  anova_groups = [anova_groups;i*ones(length(results(i).time),1)];
end
...
anova1(anova_data, anova_groups);
